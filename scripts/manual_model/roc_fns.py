...
def roc_ovr_all(y_test, y_proba, class_map, figsize=(40, 5)):
    ...
    for i in range(len(class_map)):
      ...
      df_aux = y_test.copy()
      df_aux['bin_label'] =  0
      df_aux.loc[df_aux['class_code']==c, 'bin_label'] = 1
      df_aux['prob'] = y_proba[:, i]

      # Calculates the ROC Coordinates and plots the ROC Curves
      tpr, fpr = get_all_roc_coordinates(df_aux['bin_label'], df_aux['prob'].tolist())
      plot_roc_curve(tpr, fpr, scatter = False, ax = ax[i])
      ax[i].set_title(f"ROC Curve: {list(class_map.values())[i]} vs. Rest", fontsize=20)

      # Calculates the ROC AUC OvR
      roc_auc_ovr[c] = roc_auc_score(df_aux['bin_label'], df_aux['prob'].tolist())
